package dev.drewhamilton.androidtime.format

import android.os.Build
import com.google.common.truth.Truth.assertThat
import com.google.testing.junit.testparameterinjector.TestParameter
import com.google.testing.junit.testparameterinjector.TestParameterInjector
import dev.drewhamilton.androidtime.format.test.TimeSettingTest
import java.time.LocalDate
import java.time.LocalTime
import java.time.Month
import java.time.ZoneId
import java.time.ZonedDateTime
import java.time.format.FormatStyle
import java.util.Locale
import org.junit.Assume.assumeFalse
import org.junit.Test
import org.junit.runner.RunWith

@RunWith(TestParameterInjector::class)
class AndroidDateTimeFormatterTest(
    @param:TestParameter val locale: TestLocale,
) : TimeSettingTest() {

    private val date: LocalDate = LocalDate.of(2023, Month.SEPTEMBER, 7)
    private val time: LocalTime = LocalTime.of(18, 1)
    private val dateTime: ZonedDateTime = ZonedDateTime.of(date, time, ZoneId.of("America/Chicago"))

    //region ofLocalizedTime with default style
    @Test fun ofLocalizedTime_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(context = localeContext)
        val formattedTime = formatter.format(time)
        assertThat(formattedTime).isEqualTo(locale.shortTimePreferred)
    }

    @Test fun ofLocalizedTime_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(context = localeContext)
        val formattedTime = formatter.format(time)
        assertThat(formattedTime).isEqualTo(locale.shortTime12)
    }

    @Test fun ofLocalizedTime_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(context = localeContext)
        val formattedTime = formatter.format(time)
        assertThat(formattedTime).isEqualTo(locale.shortTime24)
    }
    //endregion

    //region ofLocalizedTime with explicit style
    @Test fun ofLocalizedTime_shortFormat_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.SHORT,
        )
        val formattedTime = formatter.format(time)
        assertThat(formattedTime).isEqualTo(locale.shortTimePreferred)
    }

    @Test fun ofLocalizedTime_shortFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.SHORT,
        )
        val formattedTime = formatter.format(time)
        assertThat(formattedTime).isEqualTo(locale.shortTime12)
    }

    @Test fun ofLocalizedTime_shortFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.SHORT,
        )
        val formattedTime = formatter.format(time)
        assertThat(formattedTime).isEqualTo(locale.shortTime24)
    }

    @Test fun ofLocalizedTime_mediumFormat_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.MEDIUM,
        )
        assertThat(formatter.format(time)).isEqualTo(locale.mediumTimePreferred)
    }

    @Test fun ofLocalizedTime_mediumFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.MEDIUM,
        )
        assertThat(formatter.format(time)).isEqualTo(locale.mediumTime12)
    }

    @Test fun ofLocalizedTime_mediumFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.MEDIUM,
        )
        assertThat(formatter.format(time)).isEqualTo(locale.mediumTime24)
    }

    @Test fun ofLocalizedTime_longFormat_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.LONG,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.longTimePreferred)
    }

    @Test fun ofLocalizedTime_longFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.LONG,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.longTime12)
    }

    @Test fun ofLocalizedTime_longFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.LONG,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.longTime24)
    }

    @Test fun ofLocalizedTime_fullFormat_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.FULL,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.fullTimePreferred)
    }

    @Test fun ofLocalizedTime_fullFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.FULL,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.fullTime12)
    }

    @Test fun ofLocalizedTime_fullFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = localeContext,
            timeStyle = FormatStyle.FULL,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.fullTime24)
    }
    //endregion

    //region ofLocalizedTime with explicit locale and style
    @Test fun ofLocalizedTime_locale_shortFormat_nullSystemSetting() {
        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.SHORT,
        )
        val formattedTime = formatter.format(time)
        assertThat(formattedTime).isEqualTo(locale.shortTimePreferred)
    }

    @Test fun ofLocalizedTime_locale_shortFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.SHORT,
        )
        val formattedTime = formatter.format(time)
        assertThat(formattedTime).isEqualTo(locale.shortTime12)
    }

    @Test fun ofLocalizedTime_locale_shortFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.SHORT,
        )
        val formattedTime = formatter.format(time)
        assertThat(formattedTime).isEqualTo(locale.shortTime24)
    }

    @Test fun ofLocalizedTime_locale_mediumFormat_nullSystemSetting() {
        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.MEDIUM,
        )
        assertThat(formatter.format(time)).isEqualTo(locale.mediumTimePreferred)
    }

    @Test fun ofLocalizedTime_locale_mediumFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.MEDIUM,
        )
        assertThat(formatter.format(time)).isEqualTo(locale.mediumTime12)
    }

    @Test fun ofLocalizedTime_locale_mediumFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.MEDIUM,
        )
        assertThat(formatter.format(time)).isEqualTo(locale.mediumTime24)
    }

    @Test fun ofLocalizedTime_locale_longFormat_nullSystemSetting() {
        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.LONG,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.longTimePreferred)
    }

    @Test fun ofLocalizedTime_locale_longFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.LONG,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.longTime12)
    }

    @Test fun ofLocalizedTime_locale_longFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.LONG,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.longTime24)
    }

    @Test fun ofLocalizedTime_locale_fullFormat_nullSystemSetting() {
        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.FULL,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.fullTimePreferred)
    }

    @Test fun ofLocalizedTime_locale_fullFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.FULL,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.fullTime12)
    }

    @Test fun ofLocalizedTime_locale_fullFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedTime(
            context = testContext,
            locale = locale.value,
            timeStyle = FormatStyle.FULL,
        )
        assertThat(formatter.format(dateTime)).isEqualTo(locale.fullTime24)
    }
    //endregion

    //region ofLocalizedDate
    @Test fun ofLocalizedDate_shortFormat() {
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDate(
            context = localeContext,
            dateStyle = FormatStyle.SHORT,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.shortDate)
    }

    @Test fun ofLocalizedDate_mediumFormat() {
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDate(
            context = localeContext,
            dateStyle = FormatStyle.MEDIUM,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.mediumDate)
    }

    @Test fun ofLocalizedDate_longFormat() {
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDate(
            context = localeContext,
            dateStyle = FormatStyle.LONG,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.longDate)
    }

    @Test fun ofLocalizedDate_fullFormat() {
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDate(
            context = localeContext,
            dateStyle = FormatStyle.FULL,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.fullDate)
    }
    //endregion

    //region ofLocalizedDate with explicit locale
    @Test fun ofLocalizedDate_locale_shortFormat() {
        val formatter = AndroidDateTimeFormatter.ofLocalizedDate(
            locale = locale.value,
            dateStyle = FormatStyle.SHORT,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.shortDate)
    }

    @Test fun ofLocalizedDate_locale_mediumFormat() {
        val formatter = AndroidDateTimeFormatter.ofLocalizedDate(
            locale = locale.value,
            dateStyle = FormatStyle.MEDIUM,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.mediumDate)
    }

    @Test fun ofLocalizedDate_locale_longFormat() {
        val formatter = AndroidDateTimeFormatter.ofLocalizedDate(
            locale = locale.value,
            dateStyle = FormatStyle.LONG,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.longDate)
    }

    @Test fun ofLocalizedDate_locale_fullFormat() {
        val formatter = AndroidDateTimeFormatter.ofLocalizedDate(
            locale = locale.value,
            dateStyle = FormatStyle.FULL,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.fullDate)
    }
    //endregion

    //region ofLocalizedDateTime with dateTimeStyle
    @Test fun ofLocalizedDateTime_shortDateTimeFormat_nullSystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$shortTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_shortDateTimeFormat_12SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$shortTime12")
        }
    }

    @Test fun ofLocalizedDateTime_shortDateTimeFormat_24SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$shortTime24")
        }
    }

    @Test fun ofLocalizedDateTime_mediumDateTimeFormat_nullSystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$mediumTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_mediumDateTimeFormat_12SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$mediumTime12")
        }
    }

    @Test fun ofLocalizedDateTime_mediumDateTimeFormat_24SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$mediumTime24")
        }
    }

    @Test fun ofLocalizedDateTime_longDateTimeFormat_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$longTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_longDateTimeFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$longTime12")
        }
    }

    @Test fun ofLocalizedDateTime_longDateTimeFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$longTime24")
        }
    }

    @Test fun ofLocalizedDateTime_fullDateTimeFormat_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$fullTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_fullDateTimeFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$fullTime12")
        }
    }

    @Test fun ofLocalizedDateTime_fullDateTimeFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateTimeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$fullTime24")
        }
    }
    //endregion

    //region ofLocalizedDateTime with explicit locale and dateTimeStyle
    @Test fun ofLocalizedDateTime_locale_shortDateTimeFormat_nullSystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$shortTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_locale_shortDateTimeFormat_12SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$shortTime12")
        }
    }

    @Test fun ofLocalizedDateTime_locale_shortDateTimeFormat_24SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$shortTime24")
        }
    }

    @Test fun ofLocalizedDateTime_locale_mediumDateTimeFormat_nullSystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$mediumTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_locale_mediumDateTimeFormat_12SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$mediumTime12")
        }
    }

    @Test fun ofLocalizedDateTime_locale_mediumDateTimeFormat_24SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$mediumTime24")
        }
    }

    @Test fun ofLocalizedDateTime_locale_longDateTimeFormat_nullSystemSetting() {
        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$longTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_locale_longDateTimeFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$longTime12")
        }
    }

    @Test fun ofLocalizedDateTime_locale_longDateTimeFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$longTime24")
        }
    }

    @Test fun ofLocalizedDateTime_locale_fullDateTimeFormat_nullSystemSetting() {
        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$fullTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_locale_fullDateTimeFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$fullTime12")
        }
    }

    @Test fun ofLocalizedDateTime_locale_fullDateTimeFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateTimeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$fullTime24")
        }
    }
    //endregion

    //region ofLocalizedDateTime with dateStyle and timeStyle
    @Test fun ofLocalizedDateTime_shortDateFormat_longTimeFormat_nullSystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.SHORT,
            timeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$longTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_shortDateFormat_longTimeFormat_12SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.SHORT,
            timeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$longTime12")
        }
    }

    @Test fun ofLocalizedDateTime_shortDateFormat_longTimeFormat_24SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.SHORT,
            timeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$longTime24")
        }
    }

    @Test fun ofLocalizedDateTime_longDateFormat_shortTimeFormat_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.LONG,
            timeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$shortTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_mediumDateFormat_shortTimeFormat_12SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.MEDIUM,
            timeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$shortTime12")
        }
    }

    @Test fun ofLocalizedDateTime_mediumDateFormat_shortTimeFormat_24SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.MEDIUM,
            timeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$shortTime24")
        }
    }

    @Test fun ofLocalizedDateTime_longDateFormat_fullTimeFormat_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.LONG,
            timeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$fullTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_longDateFormat_fullTimeFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.LONG,
            timeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$fullTime12")
        }
    }

    @Test fun ofLocalizedDateTime_longDateFormat_fullTimeFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.LONG,
            timeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$fullTime24")
        }
    }

    @Test fun ofLocalizedDateTime_fullDateFormat_mediumTimeFormat_nullSystemSetting() {
        systemTimeSetting = null
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.FULL,
            timeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$mediumTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_fullDateFormat_mediumTimeFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.FULL,
            timeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$mediumTime12")
        }
    }

    @Test fun ofLocalizedDateTime_fullDateFormat_mediumTimeFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24
        testLocale = locale.value

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = localeContext,
            dateStyle = FormatStyle.FULL,
            timeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$mediumTime24")
        }
    }
    //endregion

    //region ofLocalizedDateTime with explicit locale, dateStyle, and timeStyle
    @Test fun ofLocalizedDateTime_locale_shortDateFormat_longTimeFormat_nullSystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.SHORT,
            timeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$longTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_locale_shortDateFormat_longTimeFormat_12SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.SHORT,
            timeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$longTime12")
        }
    }

    @Test fun ofLocalizedDateTime_locale_shortDateFormat_longTimeFormat_24SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 34 && locale == TestLocale.Persian)

        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.SHORT,
            timeStyle = FormatStyle.LONG,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$shortDate$shortDateTimeJoiner$longTime24")
        }
    }

    @Test fun ofLocalizedDateTime_locale_longDateFormat_shortTimeFormat_nullSystemSetting() {
        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.LONG,
            timeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$shortTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_locale_mediumDateFormat_shortTimeFormat_12SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.MEDIUM,
            timeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$shortTime12")
        }
    }

    @Test fun ofLocalizedDateTime_locale_mediumDateFormat_shortTimeFormat_24SystemSetting() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT < 31 && locale == TestLocale.France)

        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.MEDIUM,
            timeStyle = FormatStyle.SHORT,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$mediumDate$mediumDateTimeJoiner$shortTime24")
        }
    }

    @Test fun ofLocalizedDateTime_locale_longDateFormat_fullTimeFormat_nullSystemSetting() {
        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.LONG,
            timeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$fullTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_locale_longDateFormat_fullTimeFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.LONG,
            timeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$fullTime12")
        }
    }

    @Test fun ofLocalizedDateTime_locale_longDateFormat_fullTimeFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.LONG,
            timeStyle = FormatStyle.FULL,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$longDate$longDateTimeJoiner$fullTime24")
        }
    }

    @Test fun ofLocalizedDateTime_locale_fullDateFormat_mediumTimeFormat_nullSystemSetting() {
        systemTimeSetting = null

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.FULL,
            timeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$mediumTimePreferred")
        }
    }

    @Test fun ofLocalizedDateTime_locale_fullDateFormat_mediumTimeFormat_12SystemSetting() {
        systemTimeSetting = TIME_SETTING_12

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.FULL,
            timeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$mediumTime12")
        }
    }

    @Test fun ofLocalizedDateTime_locale_fullDateFormat_mediumTimeFormat_24SystemSetting() {
        systemTimeSetting = TIME_SETTING_24

        val formatter = AndroidDateTimeFormatter.ofLocalizedDateTime(
            context = testContext,
            locale = locale.value,
            dateStyle = FormatStyle.FULL,
            timeStyle = FormatStyle.MEDIUM,
        )

        val result = formatter.format(dateTime)
        with(locale) {
            assertThat(result).isEqualTo("$fullDate$longDateTimeJoiner$mediumTime24")
        }
    }
    //endregion

    //region ofSkeleton
    @Test fun ofSkeleton_MMMMdAndContext() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT == 29 && locale == TestLocale.Persian)

        testLocale = locale.value
        val formatter = AndroidDateTimeFormatter.ofSkeleton(
            skeleton = "MMMMd",
            context = localeContext,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.skeletonMMMMd)
    }

    @Test fun ofSkeleton_MMMMdAndLocale() {
        // Behavior is inconsistent on older OS levels:
        assumeFalse(Build.VERSION.SDK_INT == 29 && locale == TestLocale.Persian)

        val formatter = AndroidDateTimeFormatter.ofSkeleton(
            skeleton = "MMMMd",
            locale = locale.value,
        )
        assertThat(formatter.format(date)).isEqualTo(locale.skeletonMMMMd)
    }
    //endregion

    @Suppress("unused")
    enum class TestLocale(
        val value: Locale,
        private val preferredTimeSetting: String,
        val shortTime12: String,
        val shortTime24: String,
        val mediumTime12: String,
        val mediumTime24: String,
        val longTime12: String,
        val longTime24: String,
        val fullTime12: String,
        val fullTime24: String,
        val shortDate: String,
        val mediumDate: String,
        val longDate: String,
        val fullDate: String,
        val skeletonMMMMd: String,
        val dateTimeJoiner: String = " ",
        val shortDateTimeJoiner: String = dateTimeJoiner,
        val mediumDateTimeJoiner: String = dateTimeJoiner,
        val longDateTimeJoiner: String = dateTimeJoiner,
    ) {
        US(
            value = Locale.US,
            preferredTimeSetting = TIME_SETTING_12,
            shortTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01\u202fPM"
                else -> "6:01 PM"
            },
            shortTime24 = "18:01",
            mediumTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM"
                else -> "6:01:00 PM"
            },
            mediumTime24 = "18:01:00",
            longTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM CDT"
                else -> "6:01:00 PM CDT"
            },
            longTime24 = "18:01:00 CDT",
            fullTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM Central Daylight Time"
                else -> "6:01:00 PM Central Daylight Time"
            },
            fullTime24 = "18:01:00 Central Daylight Time",
            shortDate = "9/7/23",
            mediumDate = "Sep 7, 2023",
            longDate = "September 7, 2023",
            fullDate = "Thursday, September 7, 2023",
            skeletonMMMMd = "September 7",
            dateTimeJoiner = ", ",
            longDateTimeJoiner = " at ",
        ),
        Italy(
            value = Locale.ITALY,
            preferredTimeSetting = TIME_SETTING_24,
            shortTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01\u202fPM"
                Build.VERSION.SDK_INT >= 23 -> "6:01 PM"
                else -> "06:01 PM"
            },
            shortTime24 = "18:01",
            mediumTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM"
                else -> "6:01:00 PM"
            },
            mediumTime24 = "18:01:00",
            longTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM GMT-05:00"
                else -> "6:01:00 PM GMT-05:00"
            },
            longTime24 = "18:01:00 GMT-05:00",
            fullTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM Ora legale centrale USA"
                else -> "6:01:00 PM Ora legale centrale USA"
            },
            fullTime24 = "18:01:00 Ora legale centrale USA",
            shortDate = "07/09/23",
            mediumDate = when {
                Build.VERSION.SDK_INT >= 28 -> "7 set 2023"
                else -> "07 set 2023"
            },
            longDate = "7 settembre 2023",
            fullDate = "giovedì 7 settembre 2023",
            skeletonMMMMd = "7 settembre",
            dateTimeJoiner = ", ",
            longDateTimeJoiner = when {
                Build.VERSION.SDK_INT >= 34 -> " alle ore "
                else -> " "
            },
        ),
        France(
            value = Locale.FRANCE,
            preferredTimeSetting = TIME_SETTING_24,
            shortTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01\u202fPM"
                else -> "6:01 PM"
            },
            shortTime24 = "18:01",
            mediumTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM"
                else -> "6:01:00 PM"
            },
            mediumTime24 = "18:01:00",
            longTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM GMT-05:00"
                else -> "6:01:00 PM GMT-05:00"
            },
            longTime24 = "18:01:00 GMT-05:00",
            fullTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM heure d’été du centre nord-américain"
                else -> "6:01:00 PM heure d’été du Centre"
            },
            fullTime24 = when {
                Build.VERSION.SDK_INT >= 34 -> "18:01:00 heure d’été du centre nord-américain"
                else -> "18:01:00 heure d’été du Centre"
            },
            shortDate = "07/09/2023",
            mediumDate = "7 sept. 2023",
            longDate = "7 septembre 2023",
            fullDate = "jeudi 7 septembre 2023",
            skeletonMMMMd = "7 septembre",
            dateTimeJoiner = " à ",
            shortDateTimeJoiner = " ",
            mediumDateTimeJoiner = when {
                Build.VERSION.SDK_INT >= 31 -> ", "
                else -> " "
            },
        ),
        CanadaFrench(
            value = Locale.CANADA_FRENCH,
            preferredTimeSetting = TIME_SETTING_24,
            shortTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6 h 01\u202fp.m."
                Build.VERSION.SDK_INT >= 28 -> "6 h 01 p.m."
                else -> "6:01 p.m."
            },
            shortTime24 = "18 h 01",
            mediumTime12 = when {
                Build.VERSION.SDK_INT >= 28 -> "6 h 01 min 00 s p.m."
                else -> "6:01:00 p.m."
            },
            mediumTime24 = when {
                Build.VERSION.SDK_INT >= 28 -> "18 h 01 min 00 s"
                else -> "18:01:00"
            },
            longTime12 = when {
                Build.VERSION.SDK_INT >= 28 -> "6 h 01 min 00 s p.m. HAC"
                else -> "6:01:00 p.m. HAC"
            },
            longTime24 = when {
                Build.VERSION.SDK_INT >= 28 -> "18 h 01 min 00 s HAC"
                else -> "18:01:00 HAC"
            },
            fullTime12 = when {
                Build.VERSION.SDK_INT >= 28 -> "6 h 01 min 00 s p.m. heure avancée du Centre"
                else -> "6:01:00 p.m. heure avancée du Centre"
            },
            fullTime24 = when {
                Build.VERSION.SDK_INT >= 28 -> "18 h 01 min 00 s heure avancée du Centre"
                else -> "18:01:00 heure avancée du Centre"
            },
            shortDate = when {
                Build.VERSION.SDK_INT >= 30 -> "2023-09-07"
                else -> "23-09-07"
            },
            mediumDate = "7 sept. 2023",
            longDate = "7 septembre 2023",
            fullDate = "jeudi 7 septembre 2023",
            skeletonMMMMd = "7 septembre",
            mediumDateTimeJoiner = when {
                Build.VERSION.SDK_INT >= 31 -> ", "
                else -> " "
            },
            longDateTimeJoiner = " à ",
        ),
        Japan(
            value = Locale.JAPAN,
            preferredTimeSetting = TIME_SETTING_24,
            shortTime12 = "午後6:01",
            shortTime24 = "18:01",
            mediumTime12 = "午後6:01:00",
            mediumTime24 = "18:01:00",
            longTime12 = "午後6:01:00 GMT-05:00",
            longTime24 = "18:01:00 GMT-05:00",
            fullTime12 = "午後6:01:00 アメリカ中部夏時間",
            fullTime24 = "18時01分00秒 アメリカ中部夏時間",
            shortDate = "2023/09/07",
            mediumDate = "2023/09/07",
            longDate = "2023年9月7日",
            fullDate = "2023年9月7日木曜日",
            skeletonMMMMd = "9月7日",
        ),
        Russian(
            value = localeOf(language = "ru"),
            preferredTimeSetting = TIME_SETTING_24,
            shortTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01\u202fPM"
                Build.VERSION.SDK_INT >= 28 -> "6:01 PM"
                else -> "6:01 ПП"
            },
            shortTime24 = "18:01",
            mediumTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM"
                Build.VERSION.SDK_INT >= 28 -> "6:01:00 PM"
                else -> "6:01:00 ПП"
            },
            mediumTime24 = "18:01:00",
            longTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM GMT-05:00"
                Build.VERSION.SDK_INT >= 28 -> "6:01:00 PM GMT-05:00"
                else -> "6:01:00 ПП GMT-05:00"
            },
            longTime24 = "18:01:00 GMT-05:00",
            fullTime12 = when {
                Build.VERSION.SDK_INT >= 34 -> "6:01:00\u202fPM Центральная Америка, летнее время"
                Build.VERSION.SDK_INT >= 28 -> "6:01:00 PM Центральная Америка, летнее время"
                else -> "6:01:00 ПП Центральная Америка, летнее время"
            },
            fullTime24 = "18:01:00 Центральная Америка, летнее время",
            shortDate = "07.09.2023",
            mediumDate = when {
                Build.VERSION.SDK_INT >= 34 -> "7 сент. 2023\u202fг."
                else -> "7 сент. 2023 г."
            },
            longDate = when {
                Build.VERSION.SDK_INT >= 34 -> "7 сентября 2023\u202fг."
                else -> "7 сентября 2023 г."
            },
            fullDate = when {
                Build.VERSION.SDK_INT >= 34 -> "четверг, 7 сентября 2023\u202fг."
                else -> "четверг, 7 сентября 2023 г."
            },
            skeletonMMMMd = "7 сентября",
            dateTimeJoiner = ", ",
            longDateTimeJoiner = when {
                Build.VERSION.SDK_INT >= 34 -> " в "
                else -> ", "
            }
        ),
        Persian(
            value = localeOf(language = "fa"),
            preferredTimeSetting = TIME_SETTING_24,
            shortTime12 = "6:01 بعدازظهر",
            shortTime24 = "18:01",
            mediumTime12 = "6:01:00 بعدازظهر",
            mediumTime24 = "18:01:00",
            longTime12 = "6:01:00 بعدازظهر GMT-05:00",
            longTime24 = "18:01:00 (GMT-05:00)",
            fullTime12 = "6:01:00 بعدازظهر وقت تابستانی مرکز امریکا",
            fullTime24 = "18:01:00 (وقت تابستانی مرکز امریکا)",
            shortDate = "2023/9/7",
            mediumDate = "7 سپتامبر 2023",
            longDate = "7 سپتامبر 2023",
            fullDate = "پنجشنبه 7 سپتامبر 2023",
            skeletonMMMMd = "7 سپتامبر",
            dateTimeJoiner = when {
                Build.VERSION.SDK_INT >= 34 -> "، "
                else -> "،\u200F "
            },
            shortDateTimeJoiner = when {
                Build.VERSION.SDK_INT >= 34 -> ", "
                else -> "، "
            },
            longDateTimeJoiner = when {
                Build.VERSION.SDK_INT >= 34 -> " ساعت "
                else -> "، ساعت "
            },
        ),
        ;

        val shortTimePreferred: String
            get() = when (preferredTimeSetting) {
                TIME_SETTING_12 -> shortTime12
                TIME_SETTING_24 -> shortTime24
                else -> throw AssertionError("Invalid preferred time setting: $preferredTimeSetting")
            }

        val mediumTimePreferred: String
            get() = when (preferredTimeSetting) {
                TIME_SETTING_12 -> mediumTime12
                TIME_SETTING_24 -> mediumTime24
                else -> throw AssertionError("Invalid preferred time setting: $preferredTimeSetting")
            }

        val longTimePreferred: String
            get() = when (preferredTimeSetting) {
                TIME_SETTING_12 -> longTime12
                TIME_SETTING_24 -> longTime24
                else -> throw AssertionError("Invalid preferred time setting: $preferredTimeSetting")
            }

        val fullTimePreferred: String
            get() = when (preferredTimeSetting) {
                TIME_SETTING_12 -> fullTime12
                TIME_SETTING_24 -> fullTime24
                else -> throw AssertionError("Invalid preferred time setting: $preferredTimeSetting")
            }
    }

    private companion object {
        fun localeOf(language: String): Locale = if (Build.VERSION.SDK_INT < 36) {
            @Suppress("DEPRECATION")
            Locale(language)
        } else {
            Locale.of(language)
        }
    }
}
